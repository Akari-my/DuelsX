...
class DeclineDuelArgument implements SubCommand {
    ...
    public function execute(CommandSender $sender, array $args): void {
        ...
        $declinedMsg = str_replace("{player}", $challengerName, $messages->getNested("messages.duel.declined"));
        $sender->sendMessage($declinedMsg);

        $challenger = Main::getInstance()->getServer()->getPlayerExact($challengerName);
        if ($challenger !== null) {
            $refusedMsg = str_replace("{player}", $sender->getName(), $messages->getNested("messages.duel.refused"));
            $challenger->sendMessage($refusedMsg);
        }
    }
}
